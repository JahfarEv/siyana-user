...
Please confirm my order. Thank you! 😊`;
  ...
  const handleBranchSelect = async (branch: Branch): Promise<void> => {
    ...
    try {
      ...
      // Success modal removed - just clear cart and show branch selection is closed
      localStorage.removeItem("siyana-cart-count");
    } catch (error: any) {
      console.error("Checkout error:", error);
      alert(error.message || "Failed to process checkout. Please try again.");
...
//   const savings = cart.reduce((t, item) => {
//     if (item.originalPrice) {
//       return t + (item.originalPrice - item.price) * item.quantity;
//     }
//     return t;
//   }, 0);
...
//       const branchCode = branch.id.slice(0, 3).toUpperCase();
//       const orderId = `SIY-${timestamp}-${branchCode}${randomChars}`;

//       const message = encodeURIComponent(
//         generateOrderMessage(branch, orderId)
//       );
...
//             <Link
//               href="/"
//               className="inline-flex items-center gap-2 bg-[#196b7a] text-white px-6 py-3 rounded-lg"
//             >
//               <Plus className="w-4 h-4" />
//               Continue Shopping
//             </Link>
...
//                 <div className="text-center p-2 bg-gray-50 rounded-lg border text-xs">
//                   <CheckCircle className="w-4 h-4 text-[#196b7a] mx-auto mb-1" />
//                   <p className="font-medium">BIS</p>
//                 </div>

//                 <div className="text-center p-2 bg-gray-50 rounded-lg border text-xs">
//                   <Gem className="w-4 h-4 text-[#196b7a] mx-auto mb-1" />
//                   <p className="font-medium">IGI/GIA</p>
//                 </div>

//                 <div className="text-center p-2 bg-gray-50 rounded-lg border text-xs">
//                   <Sparkles className="w-4 h-4 text-[#196b7a] mx-auto mb-1" />
...
//             key={branch.id}
//             onClick={() => onSelect(branch)}
//             className="w-full p-4 border rounded-lg mb-3 text-left hover:border-[#196b7a]"
//           >
...
//         <button onClick={onClose} className="w-full mt-3 text-gray-600">
//           Cancel
//         </button>
//       </motion.div>
//     </div>
